...
# Library for formatting values at end
import locale
...
def getCashFlows(ticker, num_points):
    url = urlopen(CASH_FLOW_URL + ticker + QUARTER_STRING)
...
def getDiscount(ticker):
    beta = json.loads(urlopen(BETA_URL + ticker).read().decode())
    cost_of_equity = 2.1 + 6.4 * float(beta.get("profile").get("beta"))

    income_statement = json.loads(urlopen(INCOME_STATEMENT_URL + ticker).read().decode())
    balance_sheet = json.loads(urlopen(BALANCE_SHEET_URL + ticker).read().decode())
    ...
    if total_debt == 0:
        ...
        metrics = json.loads(urlopen(METRICS_URL + ticker).read().decode())
...
def getShareNumber(ticker):
    enterprise = json.loads(urlopen(ENTERPRISE_URL + ticker).read().decode())
...
def getDCF(ticker):
    ...
    shareValueDifference = ((DCF / getShareNumber(ticker)) - float(current_stock)) / (((DCF / getShareNumber(ticker)) + float(current_stock)) / 2) * 100
    evalutionDifference = (DCF - float(market_cap)) / (DCF + float(market_cap)/ 2) * 100
    return {'projected_free_cash_flow':projection, 'discount_rate': round(discount_rate, 5), 'terminal_value':'{:,.0f}'.format(terminal), 'DCF': '{:,.0f}'.format(DCF), 'share_value':'{:,.4f}'.format((DCF / getShareNumber(ticker))), 'EBIDTA': '{:,.0f}'.format((terminal // 10)), 'market_share_value': '{:,.4f}'.format(float(current_stock)), 'market_cap': '{:,.0f}'.format(float(market_cap)), 'share_diff': shareValueDifference, 'eval_diff': evalutionDifference}
